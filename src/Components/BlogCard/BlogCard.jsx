...
export const BlogCardSkeleton = () => {
  ...
  return (
    <motion.div
      ...
      className="relative bg-white backdrop-blur-md p-6 rounded-3xl shadow-xl flex flex-col justify-between"
    >
      <div className="h-6 bg-gray-200 rounded w-3/4 mb-3"></div>
      <div className="h-1 bg-gray-300 rounded w-1/4 mb-6"></div>
      <div className="flex-1 space-y-3">
        <div className="h-3 bg-gray-200 rounded w-full"></div>
        <div className="h-3 bg-gray-200 rounded w-full"></div>
        <div className="h-3 bg-gray-200 rounded w-5/6"></div>
      </div>
      <div className="mt-auto h-10 bg-gray-300 rounded-3xl w-1/2"></div>
    </motion.div>
  );
};

// Original BlogCard component
const BlogCard = ({ blog }) => {
  return (
    <motion.div
      whileHover={{ scale: 1.05, boxShadow: "0 25px 60px rgba(59,130,246,0.25)" }}
      className="relative bg-white backdrop-blur-md p-6 rounded-3xl shadow-xl transition cursor-pointer flex flex-col justify-between hover:-translate-y-1"
    >
